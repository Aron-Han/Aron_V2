...
namespace Aron_V2
{
    public partial class Camera : Form
    {
        ...
        private void Btn_Save_Click(object sender, EventArgs e)
        {
            try
            {
                Path = Global.Vpp_Root + "\\" + this.Cbo_JobID.SelectedItem.ToString() + "\\" + this.Cbo_Camera.SelectedItem.ToString() + "\\" + this.Cbo_Position.SelectedItem.ToString() + "\\Camera.vpp";
                CogSerializer.SaveObjectToFile(this.cogAcqFifoEditV21.Subject, Path);
            ...
            {
                LogChangeEventArgs.Set("Log", "Save Failed:" + m.Message, Color.Red);
            }
		...
		{
			Load_Job.VppCache.TryGetAcq(Cbo_JobID.SelectedItem as string, int.Parse(Cbo_Channel.SelectedItem as string), Cbo_Camera.SelectedItem as string, Cbo_Position.SelectedItem as string, out var Acq);
